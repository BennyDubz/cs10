	...
	private static BufferedImage thumbnailify(BufferedImage image) {
		...
		for (int x = 0; x < thumbWidth; x++) {
			for (int y = 0; y < thumbHeight; y++) {
				int imageScaledRGB = image.getRGB(x * image.getWidth() / thumbWidth, y * image.getHeight() / thumbHeight);
				result.setRGB(x, y, imageScaledRGB);
			}
		}
		return result;
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				for (int i=0; i<trows; i++) {
					for (int j=0; j<tcols; j++) {
						images.add(loadImage("recitation/recit1/dart/dart" + (i*tcols+j) + ".jpg"));
					}
				}
